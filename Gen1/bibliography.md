# Общая библиография

## Фундаментальные работы

- Vaswani et al., «Attention Is All You Need», 2017 — Transformer.
- Hoffmann et al., «Training Compute-Optimal Large Language Models» (Chinchilla), 2022 — scaling laws.
- Brown et al., «Language Models are Few-Shot Learners» (GPT-3), 2020 — in-context learning.
- Wei et al., «Chain-of-Thought Prompting Elicits Reasoning in LLMs», 2022.
- Liu et al., «Lost in the Middle: How Language Models Use Long Contexts», 2023.
- Lewis et al., «Retrieval-Augmented Generation for Knowledge-Intensive NLP Tasks», 2020.

## Книги по разработке и SRE

- Beyer et al., «Site Reliability Engineering», O'Reilly, 2016.
- Majors et al., «Observability Engineering», O'Reilly, 2022.
- Beck, «Test-Driven Development: By Example», 2002.
- Meszaros, «xUnit Test Patterns», 2007.
- Myers et al., «The Art of Software Testing», 2011.
- Khorikov, «Unit Testing: Principles, Practices, and Patterns», 2020.
- Nygard, «Documenting Architecture Decisions», 2011.
- Brown, «Software Architecture for Developers», 2018.
- Procida, «Diátaxis», 2017+.

## Безопасность и AI-риски

- Perry et al., «Do Users Write More Insecure Code with AI Assistants?», CCS 2023.
- Greshake et al., «Compromising Real-World LLM-Integrated Applications with Indirect Prompt Injection», 2023.
- OWASP LLM Top 10, 2025.

## Отраслевая эмпирика

- METR, «Measuring the Impact of AI on Experienced Open-Source Developer Productivity», 2025.
- DORA Report, 2024–2025.
- GitHub State of the Octoverse, 2024–2025.

## Регулярные источники

- [Artificial Analysis](https://artificialanalysis.ai) — бенчмарки и цены моделей.
- [LMArena](https://lmarena.ai) — pairwise human preference rankings.
- [SWE-bench](https://www.swebench.com) — кодинг-агенты.
- [Anthropic Engineering Blog](https://www.anthropic.com/engineering).
- [Simon Willison's Weblog](https://simonwillison.net).
