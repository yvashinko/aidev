...
- **Модуль 7 (Локальные модели и RAG)** — даёт альтернативу frontier-моделям там, где **логи не должны покидать периметр** (PII, compliance, sensitive PI). RAG-индекс над runbook'ами и историей постмортемов превращает «модель видит этот инцидент впервые» в «модель видит ваши прошлые 50 инцидентов».
