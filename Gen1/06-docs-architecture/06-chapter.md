...
> **See also.** §6.2 (`AGENTS.md` как анти-cutoff-щит) · §6.3 (Cold-Start CI как drift-check для README) · §6.4 (OpenAPI как auto-generated reference) · §6.8 (CI-gates для документации) · Глава 1, §1.x (knowledge cutoff в общем) · Глава 7 (локальные модели — частичное решение cutoff-проблемы за счёт RAG над репо).
